...
def load_system_config(first_time=False):
    ...
    try:
        ...
        for k in list(return_dict.keys()):
            if k in list(status_dict.keys()):
                if k in ['disks', 'interfaces', 'memory']:
                    for component in list(return_dict[k].keys()):
                        if component in list(status_dict[k].keys()):
                            for component_key in list(status_dict[k][component].keys()):
                                if component_key not in list(return_dict[k][component].keys()):
                                    return_dict[k][component][component_key] = status_dict[k][component][component_key]
                for component in list(status_dict[k].keys()):
                    if component not in list(return_dict[k].keys()):
                        return_dict[k][component] = status_dict[k][component]

        for k in list(status_dict.keys()):
            if k not in list(return_dict.keys()):
...
def update_org_info(info_dict):
    ...
    try:
        ...
        fields = ['org_name',
                  ...
                  'subunit_id']
        for field in fields:
            if field not in list(info_dict.keys()):
