...
def get_query_cursor_and_connection(db_path, query, parse_decl_types=False):
    """Get and return the cursor from the db after executing the query"""
    cur = None
    conn = None
    try:
        if parse_decl_types:
        ...
        return None, 'Error returning database cursor : %s' % str(e)
    else:
        return (cur, conn), None

def get_next_row(cur):
    """Get and return a single row from the db"""
    d = {}
    try:
        r = cur.fetchone()
        ...
        return None, 'Error reading next row from database : %s' % str(e)
    else:
        return d, None
...
def close_connection(conn):
    try:
        ...
        return False, 'Error closing database connection: %s' % str(e)
    else:
        return True, None
