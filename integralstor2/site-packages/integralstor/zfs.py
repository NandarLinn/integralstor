...
def get_exposed_ds_zvol_properties(properties):
    ...
    try:
        ...
        for prop_name in exposed_ds_zvol_properties:
            if prop_name in list(properties.keys()):
        ...
        for prop_name, prop in list(exposed_properties.items()):
            if prop_name not in readonly_properties:
                modifiable_properties[prop_name] = prop
        for prop_name in list(exposed_properties.keys()):
...
def create_dataset(parent, ds_name, properties):
    try:
        ...
        cmd = '/sbin/zfs create  '
        if properties:
            for pname, pvalue in list(properties.items()):
...
def get_free_disks(disk_type=None, spares_are_free_disks=False):
    ...
    try:
        ...
        disk_id_list = []
        for sn, disk in list(all_disks.items()):
            if 'id' in disk:
        ...
        for pool in pools:
            ...
            if not spares_are_free_disks:
                ...
                if 'spares' in pool['config'] and pool['config']['spares']:
                    ud = list(pool['config']['spares'].keys())
                    if ud:
                        used_disks.extend(ud)
            else:
                ...
                # free disks
                if 'spares' in pool['config'] and pool['config']['spares']:
                    for did, status in list(pool['config']['spares'].items()):
                        if status != 'AVAIL':
...
def get_free_disks_for_spares(pool_name):
    ...
    try:
        ...
        pool_spares = None
        if 'spares' in pool['config'] and pool['config']['spares']:
            pool_spares = list(pool['config']['spares'].keys())
...
def get_pool_spares(pool_name):
    ...
    try:
        pool, err = get_pool(pool_name)
        if 'spares' in pool['config'] and pool['config']['spares']:
            spares = list(pool['config']['spares'].keys())
    except Exception as e:
