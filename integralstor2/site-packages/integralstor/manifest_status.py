...
def get_disk_status():
    ...
    try:
        ...
        if return_dict:
            pool_list, err = zfs.get_pools()
            if pool_list:
                for sn, disk in list(return_dict.items()):
                    # print disk['name']
...
def get_interface_info():
    ...
    try:
        ...
        # print ifaces
        if ifaces:
            for iface_name, iface in list(ifaces.items()):
                td = {}
...
def get_services_status():
    ...
    try:
        ...
        if err:
            raise Exception(err)
        for service_name, service_info in list(service_dict.items()):
...
def generate_status_info(path):
    ...
    try:
        ...
        for disk_sn, disk_info_dict in list(manifest_dict["disks"].items()):
            # print disk_info_dict
            # print disk_sn
            dd = {}
            if disk_sn in list(disk_status_dict.keys()):
                for k in list(disk_status_dict[disk_sn].keys()):
        ...
        if services_status_dict:
            for service_name, service_info in list(services_status_dict.items()):
                if service_info[0] != 0:
                    status_dict['errors'].append(
        ...
        for ifname, ifdict in list(manifest_dict["interfaces"].items()):
        ...
        for ifname, ifinfo in list(interface_status_dict.items()):
        ...
        for ifname, id in list(status_dict['interfaces'].items()):
        ...
        if component_status_dict:
            for pool_name, component_status_list in list(component_status_dict.items()):
                msg = None
                severity_type_id = 1
...
def main():
    ...
    # pp.pprint(get_interface_status())
    # print zfs_version()
    # generate_status_info('/opt/integralstor/integralstor2/config/status/master.manifest')
    # pp.pprint(get_disk_status())
    # pp.pprint(generate_status_info(
    #    '/opt/integralstor/integralstor2/config/status/master.manifest'))
    # pp.pprint(generate_status_info('/opt/integralstor/integralstor2_gridcell/config/status/master.manifest'))
    # pp.pprint(generate_status_info(
    #    '/opt/integralstor/integralstor2/config/status/master.manifest'))
    #r, err = generate_status_info('/opt/integralstor/integralstor2/config/status/master.manifest')
    # print r, err
    # if err:
    #  print err
    # print r.keys()
