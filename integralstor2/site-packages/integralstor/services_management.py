...
def get_services_list():
    services = [('ntp', 'NTP'), ('smb', 'CIFS - smb'), ('winbind', 'CIFS - winbind'), ('tgtd', 'ISCSI'), ('netatalk', 'AFP services'),
                ('nfs', 'NFS'), ('vsftpd', 'FTP'), ('shellinabox', 'Shell Access'), ('urbackupsrv', 'UrBackup Server')]
    if services:
...
def get_sysd_service_status(service):
    ...
    try:
        ...
        if out:
            d['status']['output_str'] += b','.join(out).decode("utf-8")
        err2, e = command.get_error_list(ret)
        if e:
            raise Exception(e)
        if err2:
            d['status']['output_str'] += b','.join(err2).decode("utf-8")
        ...
        if not err:
            ...
            if out:
                d['is_failed']['output_str'] += b','.join(out).decode("utf-8")
            ...
            if rc == 0:
                ...
                if err:
                    raise Exception(err)
                if out:
                    d['is_active']['output_str'] += b','.join(out).decode("utf-8")
        ...
        if out:
            d['is_enabled']['output_str'] += b','.join(out).decode("utf-8")
...
def get_service_status(service):
    ...
    try:
        ...
        if err:
            raise Exception(err)
        if out:
            d['output_str'] += b','.join(out).decode("utf-8")
        err, e = command.get_error_list(ret)
        if e:
            raise Exception(e)
        if err:
            d['output_str'] += b','.join(err).decode("utf-8")
    except Exception as e:
        return None, 'Error retrieving service status : %s' % str(e)
    else:
        return d, None
