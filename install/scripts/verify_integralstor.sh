...
LIB_PATH="/usr/local/lib/python2.7/site-packages/integralstor"
...
if [ ! -f /etc/zfs/zed.d/scrub_finish-integralstor.sh ]; then
	...
	ln -s /etc/zfs/zed.d/scrub_finish-integralstor.sh /etc/zfs/zed.d/scrub_finish-integralstor.sh > /dev/null 2>&1
else
	echo "scrub_finish-integralstor.sh - Ok"
fi

if [ ! -f /etc/zfs/zed.d/resilver_finish-integralstor.sh ]; then
	...
	ln -s /etc/zfs/zed.d/resilver_finish-integralstor.sh /etc/zfs/zed.d/resilver_finish-integralstor.sh > /dev/null 2>&1
else
...
if [ ! -f /etc/BAK.shellinaboxd ]; then
	echo "shellinaboxd not copied, copying now.."
	mv /etc/shellinabox /etc/BAK.shellinaboxd
...
if [ ! -e /etc/systemd/system/uwsginew.service ]; then
	echo "/etc/systemd/system/uwsginew.service not present, copying now.."
...
if [ ! -e /etc/vsftpd/BAK.vsftpd.conf ]; then
	...
	mv /etc/vsftpd.conf /etc/vsftpd/BAK.vsftpd.conf
	cp $services_dir/vsftpd.conf /etc/vsftpd.conf
else
	echo "vsftp - Ok"
fi
