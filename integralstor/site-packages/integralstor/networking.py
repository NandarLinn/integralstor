        ...
        if init_type not in ['systemd', 'init']:
            ...
            with open('/etc/modprobe.d/bonding.conf', 'w') as f:
                if 'by_master' in bid and bid['by_master']:
                    for bn, binfo in list(bid['by_master'].items()):
                        if bn == bond_name:
                            continue
            ...
            for slave in bid['by_master'][bond_name]['slaves']:
                if os.path.isfile('/etc/network/interfaces.d/ifcfg-%s' % slave):
                    shutil.copyfile('/etc/network/interfaces.d/ifcfg-%s' %
                                    slave, '/etc/network/interfaces.d/BAK.ifcfg-%s' % slave)
                with open('/etc/network/interfaces.d/ifcfg-%s' % slave, 'w') as f:
                    f.write('DEVICE=%s\n' % slave)
                    f.write('USERCTL=no\n')
                    f.write('ONBOOT=yes\n')
                ...
                modified_file_list.append(
                    '/etc/network/interfaces.d/ifcfg-%s' % slave)
            if os.path.isfile('/etc/network/interfaces.d/ifcfg-%s' % bond_name):
                shutil.copyfile('/etc/network/interfaces.d/ifcfg-%s' %
                                bond_name, '/etc/network/interfaces.d/BAK.ifcfg-%s' % bond_name)
            if os.path.isfile('/etc/network/interfaces.d/ifcfg-%s' % bond_name):
                os.remove('/etc/network/interfaces.d/ifcfg-%s' %
                          bond_name)
            modified_file_list.append(
                '/etc/network/interfaces.d/ifcfg-%s' % bond_name)
            ...
            try:
                ...
                os.remove(
                    '/etc/network/interfaces.d/BAK.ifcfg-%s' % bond_name)
                filelist = glob.glob(
                    '/etc/network/interfaces.d/BAK.ifcfg-*')
    ...
    try:
        ...
        if init_type not in ['systemd', 'init']:
            ...
            if os.path.isfile('/etc/network/interfaces.d/ifcfg-%s' % bond_name):
                shutil.copyfile('/etc/network/interfaces.d/ifcfg-%s' %
                                bond_name, '/etc/network/interfaces.d/BAK.ifcfg-%s' % bond_name)
            with open('/etc/network/interfaces.d/ifcfg-%s' % bond_name, 'w') as f:
...
def update_bond_ip(d):
    ...
    try:
        with open('/etc/network/interfaces.d/ifcfg-bond0', 'w') as f:
            f.write('#Generated by the IntegralStor script\n')
...
def get_interfaces():
    ...
    try:
        ...
        for i in il:
            ...
            if not os.path.isfile('/etc/network/interfaces.d/ifcfg-%s' % i):
                if_dict[i] = d
                continue
        ...
        for ifname, id in list(if_dict.items()):
            if 'vlan' in id and id['vlan']:
                ...
                if vlan_comps[0] not in list(if_dict.keys()):
                    raise Exception(
...
def get_interface_bootproto(if_name):
    bootproto = 'none'
    try:
        with open('/etc/network/interfaces.d/ifcfg-%s' % if_name, 'r') as f:
            lines = f.readlines()
            for line in lines:
...
def update_interface_ip(if_name, d):
    try:
        ...
        if init_type == 'systemd':
            ...
            file_name = '/etc/network/interfaces.d/ifcfg-%s' % if_name
...
def create_vlan(if_name, vlan_id):
    try:
        ...
        file_name = '/etc/network/interfaces.d/ifcfg-%s.%d' % (
            if_name, vlan_id)
...
def delete_vlan(if_name):
    try:
        ...
        if not os.path.exists('/etc/network/interfaces.d/ifcfg-%s' % if_name):
            raise Exception('Specified VLAN does not exist')
        os.remove('/etc/network/interfaces.d/ifcfg-%s' % if_name)
...
def is_enabled_jumbo_frames(ifname):
    jumbo_frames = False
    try:
        with open('/etc/network/interfaces.d/ifcfg-%s' % ifname, 'r') as f:
            for line in f:
                if line.startswith('#'):
                    continue
                fields = line.strip().split('=')
...
def get_all_gateways():
    ...
    try:
        gd = netifaces.gateways()
        for family, gw_info in list(gd.items()):
            if family == 'default':
                continue
